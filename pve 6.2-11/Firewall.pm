...
sub ipt_rule_to_cmds {
	...
	if ($rule->{comment} && substr($rule->{comment},0,3) eq 'NAT') {
		...
		if ($rule->{type} eq 'in') {
			...
			run_command(['iptables -t nat -A PREROUTING -i ' . $ext_if . ' -p ' . $rule->{proto} . ' --dport ' . $rule->{sport} . ' -j DNAT --to ' . $rule->{dest} . ':' . $rule->{dport}]);
...
sub compile {
	...
	run_command(['iptables -t raw -I PREROUTING -i fwbr+ -j CT --zone 1']);
